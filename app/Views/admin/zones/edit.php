                ...
                <div class="card shadow" style="height: 700px;">
                    ...
                    <div class="card-footer">
                        <small class="text-muted">
                            ...
                            <strong>Instructions:</strong> Cliquez sur la carte pour modifier le centre. 
                            Utilisez les outils de dessin pour modifier le polygone de la zone.
                        </small>
                    </div>
                </div>
            </div>
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" id="latitude" name="latitude" 
                                       value="<?= old('latitude', $zone['latitude'] ?? '') ?>" placeholder="36.8065" readonly>
                            </div>

                            <div class="col-md-6">
                                <label for="longitude" class="form-label">Longitude <span class="text-danger">*</span></label>
                                <input type="text" class="form-control" id="longitude" name="longitude" 
                                       value="<?= old('longitude', $zone['longitude'] ?? '') ?>" placeholder="10.1815" readonly>
                            </div>

                            <div class="col-12">
                                <div class="alert alert-info">
                                    <i class="fas fa-info-circle me-1"></i>
                                    <strong>Cliquez sur la carte</strong> pour modifier les coordonnées GPS du centre de la zone
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
const zoneData = <?= json_encode($zone) ?>;
let zoneMap;
let drawnItems;
...
document.addEventListener('DOMContentLoaded', function() {
    // Determine initial map center - use zone coordinates if available, otherwise Tunisia default
    const initialLat = zoneData.latitude ? parseFloat(zoneData.latitude) : 36.8065;
    const initialLng = zoneData.longitude ? parseFloat(zoneData.longitude) : 10.1815;
    const initialZoom = zoneData.latitude ? 10 : 7;
    
    // Initialize map
    zoneMap = L.map('zoneMap').setView([initialLat, initialLng], initialZoom);
    ...
    if (zoneData.latitude && zoneData.longitude) {
        ...
        centerMarker.bindPopup(`<strong>Centre actuel</strong><br>Lat: ${initialLat}<br>Lng: ${initialLng}`);
    }
    ...
    // Click on map to set/update center point
    zoneMap.on('click', function(e) {
        ...
        centerMarker.bindPopup(`<strong>Nouveau centre</strong><br>Lat: ${lat}<br>Lng: ${lng}`).openPopup();
    });
});
